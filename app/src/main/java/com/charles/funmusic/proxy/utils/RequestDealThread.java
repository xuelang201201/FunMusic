...
public class RequestDealThread extends Thread {
    ...
    private ProxyFileUtils mFileUtils;

    private CacheFileInfoDao mCacheFileInfoDao;
    private Context mContext;

    RequestDealThread(Context context, HttpURLConnection request, Socket client) {
        mRequest = request;
        mClient = client;
        mCacheFileInfoDao = CacheFileInfoDao.getInstance();
        mContext = context;
    }
    ...
    public void run() {
        try {
            mFileUtils = ProxyFileUtils.getInstance(mContext, mRequest.getURL().toURI(), true);
            processRequest(mRequest, mClient);
    ...
    private int getRangeStart(HttpURLConnection request) {
        ...
        //	Header rangeHeader = mRequest.getFirstHeader(Constants.RANGE);
        if (value != null) {
    ...
    private void sendLocalHeaderAndCache(int rangeStart, int rangeEnd, int fileLength, byte[] audioCache)
        ...
        mClient.getOutputStream().write(httpHeader, 0, httpHeader.length);
        // 返回Content
        if (audioCache != null && audioCache.length > 0) {
            mClient.getOutputStream().write(audioCache, 0, audioCache.length);
        }
    }
     ...
     */
            int originRangeStart = getRangeStart(request);
            Log.i(LOG_TAG, "原始请求Range起始值：" + originRangeStart + " 本地缓存长度：" + mFileUtils.getLength());
            // 缓存的文件大小
            int cacheFileSize = mCacheFileInfoDao.getFileSize(mFileUtils.getFileName());
            /*
             * 如果缓存完成，无需发送请求，本地缓存返回MediaPlayer。
			 */
            if (mFileUtils.isEnable() && mFileUtils.getLength() == cacheFileSize) {
                audioCache = mFileUtils.read(originRangeStart, Constants.AUDIO_BUFFER_MAX_LENGTH);
     ...
     */
            long realRangeStart;
            if (mFileUtils.isEnable() && originRangeStart < mFileUtils.getLength()) {
                audioCache = mFileUtils.read(originRangeStart, Constants.AUDIO_BUFFER_MAX_LENGTH);
                Log.i(LOG_TAG, "本地已缓存长度（跳过）:" + audioCache.length);
                // 得到需要发送请求Range Start（本地缓存结尾位置+1=缓存长度）
                realRangeStart = mFileUtils.getLength();
                // 替换请求Header
                request.setRequestProperty(Constants.RANGE, Constants.RANGE_PARAMS + realRangeStart + "-");
                //	mRequest.removeHeaders(Constants.RANGE);
                //	mRequest.addHeader(Constants.RANGE, Constants.RANGE_PARAMS + realRangeStart + "-");
            } else {
            ...
            // 缓存不够。或者数据库没有文件大小
                ...
                if (!isCacheEnough) {
                    ...
                    int fileLength = 0;
                    int readBytes;
                    while (Thread.currentThread() == MusicPlayerProxy.downloadThread
                        ...
                        // 保存文件
                        if (mFileUtils.getLength() == fileBufferLocation) {
                            mFileUtils.write(buff, readBytes);
                        }
                        // 打印缓存大小
    ...
    private int getContentLength(HttpURLConnection response) {
        int contentLength;
        String range = mRequest.getHeaderField(Constants.RANGE);
        if (range != null) {
            contentLength = Integer.valueOf(range.substring(range.indexOf("-") + 1, range.indexOf("/"))) + 1;
        } else {
            contentLength = mRequest.getContentLength();
        }
        if (contentLength != 0) {
            mCacheFileInfoDao.insertOrUpdate(mFileUtils.getFileName(), contentLength);
        }
