...
public class ProxyFileUtils {
    ...
    private boolean isSdAvailable() {
        ...
        // 删除部分缓存文件
        ProxyUtils.asyncRemoveBufferFile(Constants.CACHE_FILE_NUMBER);
        // 可用空间大小是否大于SD卡预留最小值
        long freeSize = ProxyUtils.getAvailableSize(Constants.DOWNLOAD_PATH);
    ...
    public static boolean isFinishCache(String url) {
        ...
        return f.exists() && f.length() == CacheFileInfoDao.getInstance().getFileSize(name);
    }

    /**
     * 获取有效的文件名
     */
    private static String getValidFileName(URI uri) {
