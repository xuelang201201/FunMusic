...
public class CoverLoader {
    ...
    private String getKey(Music music, Type type) {
        ...
        } else if (music.getType() == Music.Type.ONLINE && !TextUtils.isEmpty(music.getCoverUrl())) {
            return music.getCoverUrl().concat(type.value);
        } else {
            return null;
        }
    }
    ...
    private Bitmap getDefaultCover(Type type) {
        switch (type) {
            ...
            case ROUND:
                ...
                return bitmap;
            default:
                return BitmapFactory.decodeResource(mContext.getResources(), R.drawable.ic_default_album_cover);
        }
    }
    ...
    private Bitmap loadCoverByType(Music music, Type type) {
        ...
        if (music.getType() == Music.Type.LOCAL) {
            ...
            bitmap = loadCoverFromFile(music.getCoverUrl());
        }
    ...
    private Bitmap loadCoverFromMediaStore(long albumId) {
        ...
        Uri uri = MusicUtil.getAlbumArtUri(albumId);
        InputStream is;
        try {
