...
public class Permission {
    ...
    private static ArrayList<PermissionRequest> sPermissionRequests = new ArrayList<>();

    public static void init(Context context) {
        sSharedPreferences = context.getSharedPreferences("pl.tajchert.runtimepermissionhelper", Context.MODE_PRIVATE);
        Permission.sContext = context;
    }
    ...
    /**
     * Get list of currently granted permissions, without saving it inside Permission
     *
    ...
    public static void ignorePermission(String permission) {
        if (!isIgnoredPermission(permission)) {
            ...
            sSharedPreferences.edit().putStringSet(KEY_IGNORED_PERMISSIONS, set).apply();
        }
    }
    ...
    public static void permissionCompare(PermissionListener permissionListener) {
        if (sContext == null) {
            throw new RuntimeException("Before comparing permissions you need to call Permission.initCatchException(context)");
    ...
    public static boolean checkPermission(String permissionName) {
        ...
        return PackageManager.PERMISSION_GRANTED == sContext.checkSelfPermission(permissionName);
    }
}
