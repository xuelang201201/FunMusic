...
public class ArtistSortFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        @SuppressLint("InflateParams") View view = LayoutInflater.from(getActivity()).inflate(R.layout.dialog_artist_sort, null);
        ButterKnife.bind(this, view);

        int sortWay = Preferences.getArtistSortWay();
    ...
    @OnClick({R.id.dialog_sort_by_artist, R.id.dialog_sort_by_song_count, R.id.dialog_sort_cancel})
    public void setSortWay(View view) {
        switch (view.getId()) {
            case R.id.dialog_sort_by_artist:
                Preferences.saveArtistSortWay(SORT_BY_ARTIST);
                mPreferences.saveArtistSortOrder(SortOrder.ArtistSortOrder.ARTIST_A_Z);
                dismiss();
                break;

            case R.id.dialog_sort_by_song_count:
                Preferences.saveArtistSortWay(SORT_BY_SONG_COUNT);
                mPreferences.saveArtistSortOrder(SortOrder.ArtistSortOrder.ARTIST_NUMBER_OF_SONGS);
                dismiss();
                break;
